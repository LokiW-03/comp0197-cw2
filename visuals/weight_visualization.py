...
from PIL import Image, ImageDraw, ImageOps, ExifTags # Added ImageDraw
...
# --- End Helper ---
# In draw_points and draw_boxes, use rounding when scaling
def draw_points(draw_context, points_list, target_size, x_scale, y_scale, color="red", radius=3):
    ...
    for x_orig, y_orig  in points_list:
        # Scale coordinates with rounding
        x_scaled = int(round(float(x_orig) * x_scale))
        y_scaled = int(round(float(y_orig) * y_scale))

        # Clamp coordinates to be within target bounds
        x_scaled = max(0, min(target_w - 1, x_scaled))
        y_scaled = max(0, min(target_h - 1, y_scaled))

        # Draw ellipse with clamped coordinates
...
def draw_boxes(draw_context, boxes_list, target_size, x_scale, y_scale, color="lime", width=1):
    ...
    for xmin_orig, ymin_orig, xmax_orig, ymax_orig  in boxes_list:
        # Scale coordinates with rounding
        xmin_scaled = int(round(float(xmin_orig) * x_scale))
        ymin_scaled = int(round(float(ymin_orig) * y_scale))
        xmax_scaled = int(round(float(xmax_orig) * x_scale))
        ymax_scaled = int(round(float(ymax_orig) * y_scale))
        ...
        if xmin_scaled > xmax_scaled:
            xmin_scaled, xmax_scaled = xmax_scaled, xmin_scaled
        if ymin_scaled > ymax_scaled:
            ymin_scaled, ymax_scaled = ymax_scaled, ymin_scaled
...
class SegmentationVisualizer:
    def __init__(self, model_paths, sample_image_path, weak_labels_path, output_dir="results"):
            ...
            if self.sample_image_filename not in self.weak_labels:
                ...
                if 'points' in self.sample_image_weak_labels:
                    self.sample_image_weak_labels['points'] = [
                        (float(p[0]), float(p[1])) for p in self.sample_image_weak_labels['points']
                    ]
                if 'scribbles' in self.sample_image_weak_labels:
                    scribbles_dict = self.sample_image_weak_labels['scribbles']
                    processed_scribbles = {}
                    for key in ['foreground', 'background']:
                        if key in scribbles_dict:
                            processed_scribbles[key] = [
                                (float(p[0]), float(p[1])) for p in scribbles_dict[key]
                            ]
                    self.sample_image_weak_labels['scribbles'] = processed_scribbles
                if 'boxes' in self.sample_image_weak_labels:
                    self.sample_image_weak_labels['boxes'] = [
                        (float(b[0]), float(b[1]), float(b[2]), float(b[3])) for b in self.sample_image_weak_labels['boxes']
                    ]
                logging.info("Converted annotation coordinates to standard Python floats.")
                 #---------------------------------------------------------------------
        ...
        try:
            # Load image to get EXIF data and original stored dimensions
            with Image.open(sample_image_path) as img:
                # Get EXIF orientation correctly
                self.original_orientation = 4
                ...
                if exif_data is not None:
                    ...
                    for tag, name in ExifTags.TAGS.items():
                        if name == 'Orientation':
                            orientation_tag = tag
                            break
                    # Get the orientation value, default to 1 if not found
                    self.original_orientation = exif_data.get(orientation_tag, 1)
                # Original stored dimensions (before EXIF correction)
                self.original_stored_width, self.original_stored_height = img.size
            
            # Reload image and apply EXIF correction
            self.sample_image_raw_full = Image.open(sample_image_path).convert('RGB')
            self.sample_image_raw_full = ImageOps.exif_transpose(self.sample_image_raw_full)
            self.orig_w, self.orig_h = self.sample_image_raw_full.size  # Corrected dimensions
            logging.info(f"EXIF Orientation: {self.original_orientation}, Corrected Size: {self.orig_w}x{self.orig_h}")
        ...
        # --- Adjust annotations for EXIF orientation ---
        if self.sample_image_weak_labels:
            # Points adjustment
            if 'points' in self.sample_image_weak_labels:
                adjusted_points = []
                for x_orig, y_orig in self.sample_image_weak_labels['points']:  # Correct order
                    x_adj, y_adj = self.adjust_coordinates_for_exif(x_orig, y_orig)
                    adjusted_points.append((x_adj, y_adj))  # Maintain (x, y)
                self.sample_image_weak_labels['points'] = adjusted_points

            # Boxes adjustment
            if 'boxes' in self.sample_image_weak_labels:
                adjusted_boxes = []
                for xmin_orig, ymin_orig, xmax_orig, ymax_orig in self.sample_image_weak_labels['boxes']:  # Correct order
                    xmin_adj, ymin_adj = self.adjust_coordinates_for_exif(xmin_orig, ymin_orig)
                    xmax_adj, ymax_adj = self.adjust_coordinates_for_exif(xmax_orig, ymax_orig)
                    # Ensure valid box coordinates
                    new_xmin = min(xmin_adj, xmax_adj)
                    new_xmax = max(xmin_adj, xmax_adj)
                    new_ymin = min(ymin_adj, ymax_adj)
                    new_ymax = max(ymin_adj, ymax_adj)
                    adjusted_boxes.append((new_xmin, new_ymin, new_xmax, new_ymax))
                self.sample_image_weak_labels['boxes'] = adjusted_boxes

        logging.info("Adjusted annotations for EXIF orientation")
    ...
    # Add helper method to adjust coordinates
    def adjust_coordinates_for_exif(self, x, y):
        """Adjust coordinates based on EXIF orientation."""
        W = self.original_stored_width  # Original dimensions (pre-correction)
        H = self.original_stored_height

        if self.original_orientation == 1:  # 0° (no rotation)
            return x, y
        elif self.original_orientation == 3:  # 180°
            return (W - x - 1), (H - y - 1)
        elif self.original_orientation == 6:  # 90° CW
            return (H - y - 1), x
        elif self.original_orientation == 8:  # 90° CCW
            return y, (W - x - 1)
        else:  # Default (unhandled orientations)
            logging.warning(f"Unsupported EXIF orientation: {self.original_orientation}")
            return x, y
        ...
        try:
            try:
                ...
                logging.info("Attempted load with weights_only=True.")
                 ...
                 checkpoints = torch.load(path, map_location='cpu', weights_only=False) # Fallback
            ...
            elif isinstance(checkpoints, dict) and 'state_dict' in checkpoints:
                state_dict = checkpoints['state_dict']
                logging.info("Extracted state_dict from 'state_dict' key.")
            elif isinstance(checkpoints, dict) and all(isinstance(k, str) and isinstance(v, torch.Tensor) for k, v in checkpoints.items()):
                state_dict = checkpoints
                logging.info("Checkpoints appears to be state_dict directly.")
            else:
                 logging.error(f"Loaded checkpoints dictionary has unexpected structure. Keys: {list(checkpoints.keys())}")
                 raise TypeError("Checkpoints format not recognized.")

        except Exception as load_e:
            logging.error(f"Failed to load model checkpoints from {path}: {load_e}", exc_info=True)
            raise load_e
    ...
    def plot_comparison_grid(self):
            ...
            if 'points' in model_key.lower():
                if points:
                    ...
                    draw_points(draw, points, self.img_size, self.x_scale, self.y_scale,
                                color="red", radius=points_radius) # Pass scales & target size
                    annotation_printed = True
                else: print(f"  points requested but none found for this image.")
            if 'scribbles' in model_key.lower():
                foreground = scribbles.get('foreground', [])
                background = scribbles.get('background', [])
                draw_points(draw, foreground, self.img_size, self.x_scale, self.y_scale, color="red", radius=3)
                draw_points(draw, background, self.img_size, self.x_scale, self.y_scale, color="blue", radius=3)
        ...
        save_path = os.path.join(self.output_dir, "oeq_model_activation.png")
        try:
...
if __name__ == "__main__":
    ...
    SAMPLE_IMAGE_PATH = "./data/images/Abyssinian_90.jpg"
    OUTPUT_DIR = "visuals" # Changed output dir name
    ...
    if abort: print("\nERROR: Required file(s) missing. Abortinhttps://file+.vscode-resource.vscode-cdn.net/"); exit()
